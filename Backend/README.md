...
- **Frontend:** [gestion-proveedores](https://github.com/OselkoNico/gestion-proveedores) (Angular 21)
...
**4. Crear el usuario administrador.** El registro público solo crea usuarios
...
El `--` es necesario para que npm pase los argumentos al script.
...
Se cargan desde `Backend/.env`, que no se versiona. `.env.example` documenta
las claves necesarias con los valores sensibles vacíos.
...
**Las restricciones viven en la base de datos.** La unicidad del CIF es la
clave primaria, la del correo un índice único, y la obligatoriedad del nombre
un `NOT NULL`. El controlador se limita a traducir el error de MySQL al código
...
interpolan **ya convertidos a entero**: `mysql2` envía los parámetros
preparados como cadenas y MySQL no las admite en esa posición.
...
sustituye por un doble. Unas pruebas que exigen un servidor de base de datos
con datos concretos son unas pruebas que nadie ejecuta y que no pueden correr
...
`limit` de cero recaía en el valor por defecto, y una página decimal generaba
un `OFFSET` no entero que MySQL rechaza, convirtiendo una entrada malformada en
un error 500.
...
- **Los tokens no se pueden revocar** antes de su caducidad. Un cierre de
  sesión solo descarta el token en el cliente. Resolverlo requeriría tokens de
  refresco o una lista de revocación.
